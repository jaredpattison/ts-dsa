...
const greeting = (word: string) => {
    return word.toUpperCase();
};

const speaker = (message: string, cb: any) => {
    return cb(message);
}
...
const addValues = (arr: any[], val: any) => {
    arr.push(val);
}

const addNumbers = (num: number, arr: number[], times: number, cb: any) => {
...
const removeOne = (num: number, arr: number[]) => {
    if (num % 3 === 2) arr.pop();
};

const removeElements = (arr: number[], cb: any) => {
    for (let i of arr) cb(i, arr);
    return arr;
}
...
const removeWithForEach = (arr: number[], cb: any) => {
    arr.forEach(num => cb(num, arr));
    return arr;
}
...
const removeWithAnon = (arr: number[]) => {
    arr.forEach(num => {
        if (num % 3 === 2) arr.pop();
    });
    return arr;
...
const fizzbuzz = (arr: number[]) => {
    const res: any[] = [];
    arr.forEach(num => {
        if (num % 3 === 0 && num % 5 === 0) return res.push("Fizz Buzz");
        if (num % 3 === 0) return res.push("Fizz");
        if (num % 5 === 0) return res.push("Buzz");
        return res.push(num);
    });
    return res;
}
